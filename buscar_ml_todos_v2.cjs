...
async function carregarProdutos() {
  ...
  if (fs.existsSync(CONFIG.PLANILHA_JSON)) {
    ...
    return dados.map(p => ({
      id:       p.rowId || p.id || '',
      nome:     p.supplierName || p.nomeSF || p.nome || p.Produto || p.name || '',
      origem:   p.origin || p.status || '',
      isMistral: p.isMistral || false,
      milaoPor: p.milaoPor || p.finalCost || 0,
      sfPor:    p.sfPor || p.sfFinal || 0,
...
async function main() {
  ...
  for (let i = 0; i < produtos.length; i++) {
    ...
    const registro = {
      isMistral:        p.isMistral,
      milao_por:        p.milaoPor,
      sf_por:           p.sfPor,
      diferenca_ml_sf:  (mlResultados.length > 0 && p.sfPor > 0) ? parseFloat((mlResultados[0].preco - p.sfPor).toFixed(2)) : null,
      produto_id:      p.id,
      nome:            p.nome,
      origem:          p.origem,
  ...
  // Top 10 — maior diferença ML menor que seu SF Por (oportunidades de compra)
  const comDif = resultados.filter(r => r.melhor_preco && r.sf_por > 0);
  const maisBaratosML = comDif
    .filter(r => r.melhor_preco < r.sf_por)
    .sort((a, b) => (a.melhor_preco - a.sf_por) - (b.melhor_preco - b.sf_por))
    .slice(0, 10);

  if (maisBaratosML.length > 0) {
    console.log('\n🔥 TOP 10 — ML MAIS BARATO QUE SEU PREÇO SF (possível defasagem!):');
    maisBaratosML.forEach((r, i) => {
      const dif = r.melhor_preco - r.sf_por;
      console.log(`  ${i+1}. ${r.nome.substring(0,38).padEnd(38)} | ML: R$ ${r.melhor_preco.toFixed(2).padStart(9)} | SF: R$ ${r.sf_por.toFixed(2).padStart(9)} | dif: R$ ${dif.toFixed(2)}`);
    });
  }

  const maisCaosML = comDif
    .filter(r => r.melhor_preco > 0)
    .sort((a, b) => b.melhor_preco - a.melhor_preco)
    .slice(0, 5);
  console.log('\n💎 TOP 5 — MAIS CAROS NO ML:');
  maisCaosML.forEach((r, i) => {
    console.log(`  ${i+1}. ${r.nome.substring(0,38).padEnd(38)} → R$ ${r.melhor_preco.toFixed(2)}`);
  });
