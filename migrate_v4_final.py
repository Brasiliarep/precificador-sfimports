...
def run_migration():
    print("🚀 Starting FINAL Master Sheet v4 Migration...")
    ...
    for i, row in enumerate(sheet.iter_rows(min_row=5, values_only=True)):
        ...
        excel_data.append({
            ...
            "sfPor": parse_price(row[4]), # Map SF POR / VENDA to sfPor
            "sfFinal": parse_price(row[5]), # Map SF POR / FINAL to sfFinal
    ...
    for item in excel_data:
        ...
        if norm_excel in json_map:
            for row in json_map[norm_excel]:
                # UPDATE SUPPLIER PRICES IF > 0
                if item["milaoPor"] > 0:
                    row["milaoPor"] = item["milaoPor"]
                    row["milaoDe"] = item["milaoDe"]
                    row["finalCost"] = item["milaoPor"]
                    row["supplierCostRaw"] = item["milaoPor"]
                    milao_price_updated += 1
                
                # UPDATE SF PRICES IF > 0 (THIS IS THE FIX)
                if item["sfPor"] > 0:
                    row["sfPor"] = item["sfPor"]
                    row["sfDe"] = item["sfPor"] # Using Por as De (Riscado) if needed
                    sf_price_updated += 1
    ...
    print(f"✅ FINAL Migration Finished!")
    print(f"✨ Total Matched/Revised: {matched}")
    print(f"💰 Milão Prices Updated: {milao_price_updated}")
    print(f"🍷 SF Prices Updated: {sf_price_updated}")
